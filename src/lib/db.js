...
//   try {
//     const p = getPool();
//     const [cols] = await p.execute("SHOW COLUMNS FROM users LIKE 'name'");
//     if (cols.length === 0) {
//       console.log("🛠️ Database Migration: Adding 'name' and 'gender' columns to users table...");
...
//   const db = getPool();
//   if (!migrationDone && !sql.includes("SHOW COLUMNS") && !sql.includes("ALTER TABLE")) {
//     await ensureSchema();
//   }
...
//   const rows = await query('SELECT id, email, role, name, gender, created_at FROM users WHERE id = ?', [id]);
//   return rows[0] || null;
// }

// // Ensure admin user exists (used by login bootstrap)
...
//   if (userId) {
//     const rows = await query('SELECT * FROM assessments WHERE user_id = ? ORDER BY created_at DESC', [userId]);
...
//   const overallScore = assessmentData.overallScore != null ? parseInt(assessmentData.overallScore) : 0;

//   if (existing.length > 0) {
//     await query(
//       `UPDATE assessments 
//        SET user_id=?, user_email=?, project_name=?, answers=?, scores=?, overall_score=?, remarks=?, remarks_provider=?, feedback=?, updated_at=NOW()
//        WHERE id=?`,
//       [
...
//   } else {
//     await query(
//       `INSERT INTO assessments (id, user_id, user_email, project_name, answers, scores, overall_score, remarks, remarks_provider, feedback)
//        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
//       [
//         id,
//         assessmentData.userId || assessmentData.user_id,
...
//   // Use stored overall_score; fall back to computing from scores if missing
...
//         [
//           questionData.area,
//           questionData.subArea,
//           questionData.practice,
//           questionData.type || 'extent',
...
function getPool() {
  if (!global.mysqlPool) {
    // ✅ IMPORTANT FIX: use Railway PUBLIC URL directly
    if (!process.env.MYSQL_PUBLIC_URL) {
      throw new Error("MYSQL_PUBLIC_URL is not defined in environment variables");
    }
...
export async function saveAssessment(data) {
  const id = data.id || 'asm_' + Math.random().toString(36).substring(2, 10);

  const existing = await query('SELECT id FROM assessments WHERE id = ?', [id]);

  const answers = JSON.stringify(data.answers || {});
  const scores = JSON.stringify(data.scores || {});
  const feedback = JSON.stringify(data.feedback || null);

  const overallScore = parseInt(data.overallScore || 0);
  ...
  if (existing.length > 0) {
    ...
    await query(
      `INSERT INTO assessments 
       (id, user_id, user_email, project_name, answers, scores, overall_score, remarks, remarks_provider, feedback)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        data.userId,
...
function parseAssessmentRow(row) {
  ...
  let overallScore = row.overall_score != null
    ? parseInt(row.overall_score)
    : Math.round(((Object.values(scores).reduce((a, b) => a + b, 0) / (Object.values(scores).length || 1)) / 5) * 100);
...
export async function saveQuestion(q) {
  if (q.id) {
    const existing = await query('SELECT id FROM questions WHERE id = ?', [q.id]);

    if (existing.length) {
      await query(
        `UPDATE questions SET area=?, sub_area=?, practice=?, type=?, question_text=? WHERE id=?`,
        [q.area, q.subArea, q.practice, q.type || 'extent', q.questionText, q.id]
      );
      return true;
    }
  }
  ...
  await query(
    `INSERT INTO questions (area, sub_area, practice, type, question_text)
     VALUES (?, ?, ?, ?, ?)`,
    [q.area, q.subArea, q.practice, q.type || 'extent', q.questionText]
  );
...
export async function deleteQuestion(id) {
  const res = await query('DELETE FROM questions WHERE id = ?', [id]);
...
export async function getSettings() {
  ...
  return {
    activeAIProvider: row.active_ai_provider,
    apiKeys: typeof row.api_keys === 'string' ? JSON.parse(row.api_keys) : {},
